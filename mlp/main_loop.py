...
for s in seed_list:
    ...
    configs["seed"] = s # random seed update
    ...
    for epoch in range(configs["epochs"]):
        ...
        print(f"Seed: {configs['seed']}] Epoch: {epoch+1:02d} | Loss: {np.mean(loss_per_epoch):.4f}")
    ...
    plt.tight_layout()
    plt.savefig(f"./fig/4_training_loss_{configs['seed']}.png")
    plt.show()
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f"./fig/4_metric_curve_{configs['seed']}.png")
    plt.show()
    plt.close()
...
results_df.to_csv("./assets/val_results.csv", index=False)

results_df = pd.DataFrame(test_results)
results_df.to_csv("./assets/test_results.csv", index=False)
#%%