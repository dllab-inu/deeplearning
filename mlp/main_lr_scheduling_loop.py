...
for s in seed_list:
    ...
    for epoch in range(configs["epochs"]):
        ...
        print(f"[Seed: {configs['seed']}] Epoch: {epoch+1:02d} | Loss: {np.mean(loss_per_epoch):.4f} | lr={current_lr:.6f}")
    ...
    plt.tight_layout()
    plt.savefig(f"./fig/4_training_loss_lr_scheduling_{configs['seed']}.png")
    plt.show()
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f"./fig/4_metric_curve_lr_scheduling_{configs['seed']}.png")
    plt.show()
    plt.close()
