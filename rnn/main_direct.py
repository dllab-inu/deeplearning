...
class Forecaster(nn.Module):
    def __init__(self, configs):
        ...
        self.decoder = nn.Linear(
            configs['hidden_dim'], configs['horizon'] * configs['feature_dim'])

    def forward(self, x):
        enc_out, h = self.encoder(x) # [B, lookback, hidden_dim], [num_layers, B, hidden_dim]

        last_hidden = enc_out[:, -1, :] # [B, hidden_dim]

        pred = self.decoder(last_hidden) # [B, horizon]

        return pred.unsqueeze(-1) # [B, horizon, feature_dim]
...
plt.tight_layout()
plt.savefig("./fig/direct_loss_curve.png")
plt.show()
plt.close()
#%%
...
plt.tight_layout()
plt.savefig("./fig/direct_forecast.png")
plt.show()
plt.close()
#%%